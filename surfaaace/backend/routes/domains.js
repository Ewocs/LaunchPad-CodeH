...
router.post('/', [
  ...
  // Update user's domain usage
  req.user.usage.domainsUsed += 1;
  await req.user.save();
...
router.put('/:id', [
  ...
  // Check if monitoring is being enabled and user has permission
  if (updates.monitoringSettings?.enabled && !req.user.planLimits.realTimeMonitoring) {
    return res.status(402).json({
      success: false,
      message: 'Real-time monitoring not available in your plan'
    });
  }
...
router.delete('/:id', [
  ...
  // Update user's domain usage
  req.user.usage.domainsUsed = Math.max(0, req.user.usage.domainsUsed - 1);
  await req.user.save();
